...
export const chapter7Missions: MissionSeed[] = [
  {
    ...
    description:
      '계산 API의 핵심 로직이 0이라는 단 하나의 입력 때문에 종료됩니다. 정수 n을 받아 10 // n을 출력하되, n이 0일 때만 error를 출력해야 합니다. 정상 입력의 동작은 보존하고 ZeroDivisionError만 정확히 처리하세요. 음수도 정상적인 피연산자입니다.',
    difficulty: 4,
    isBoss: false,
  ...
  },
  {
    ...
    title: '불완전한 입력 파이프라인',
    description:
      '사용자 입력을 정수로 변환해 두 배로 만드는 파이프라인입니다. 정수 문자열은 음수와 주변 공백까지 정상 처리하고, 정수로 해석할 수 없는 값에는 not a number를 출력해야 합니다. 예외 범위를 넓게 잡아 다른 프로그래밍 오류까지 숨기지 말고 ValueError만 처리하세요.',
    difficulty: 5,
    isBoss: false,
    bugTypeSlug: 'exception',
  ...
  },
  {
    ...
    title: '사라진 리소스의 경계',
    description:
      '외부 리소스는 언제든 사라질 수 있습니다. 입력받은 경로의 파일을 읽어 출력하되, 파일이 존재하지 않는 경우에만 missing을 출력하세요. with 문을 유지해 열린 파일이 항상 닫히게 하고, FileNotFoundError보다 넓은 예외로 실제 장애 원인을 감추지 마세요.',
    difficulty: 5,
    isBoss: false,
    bugTypeSlug: 'exception',
    initialCode:
  ...
  },
  {
    ...
    description:
      '공백으로 구분된 두 정수를 받아 첫 번째 수를 두 번째 수로 나눕니다. 토큰 부족, 숫자 변환 실패, 0 나눗셈이라는 서로 다른 실패를 invalid input이라는 하나의 사용자 계약으로 변환하세요. 정상적인 음수 연산은 그대로 통과해야 하며, 세 예외만 명시적으로 묶어 처리해야 합니다.',
    difficulty: 5,
    isBoss: false,
    bugTypeSlug: 'exception',
    initialCode:
  ...
  },
  {
    ...
    description:
      '통계 보고서 생성기가 데이터가 없는 날에만 연쇄적으로 실패합니다. n개의 정수를 읽어 합계·평균·최댓값을 출력하고, n=0이면 no data 한 줄만 출력하세요. 빈 컬렉션을 계산 함수에 넘기기 전에 가드 절로 차단해야 하며, 정상 경로의 출력 순서와 실수형 평균 형식은 바꾸면 안 됩니다.',
    difficulty: 5,
    isBoss: true,
    bugTypeSlug: 'exception',
    initialCode:
